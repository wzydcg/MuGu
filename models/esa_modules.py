...
class PPM(nn.Module):
    ...
    def forward(self, feat):
        b, c, h, w = feat.shape#b是batch_size，c是通道数，h和w是特征图的高度和宽度
...
class ESA_layer(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.):
        ...
        #判断是否需要对多头注意力的输出进行线性变换和dropout。如果`heads`为1且`dim_head`等于`dim`，则不需要额外的投影
        self.heads = heads
        ...
        self.to_out = nn.Sequential(
            ...
            nn.Dropout(dropout)
        ) if project_out else nn.Identity()#nn.Identity()不改变输入
